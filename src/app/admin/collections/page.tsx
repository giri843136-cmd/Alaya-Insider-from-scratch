...
export default function AdminCollections() {
  ...
  useEffect(() => {
    adminFetch('/api/collections').then(r => r.json()).then(d => { setCollections(d.collections || []); setLoading(false); });
  }, []);

  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-semibold text-gray-800">Collections</h1>
      </div>
          ...
          <tbody>
            ...
            collections.map(c => (
              <tr key={c.id} className="border-b border-gray-50 hover:bg-gray-50">
                ...
                <td className="p-3 text-gray-500 hidden sm:table-cell">{c.product_count}</td>
                <td className="p-3 text-right">
                  <Link href={`/collections/${c.slug}`} target="_blank" className="text-xs text-gray-400 hover:text-accent">View</Link>
                </td>
              </tr>
            ))}
          </tbody>
